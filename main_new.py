...
def main():
    ...
    # Initialize globals with desired values
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if current_state == STATE_MENU:
                ...
                if is_host and btn_start_lobby.handle_event(event):
                    ...
                    players_list = list(net_manager.peers.keys())
                    try:
                        net_manager.send_event("START_GAME", players=players_list, game=selected_game or 'battleship', seed=random.random()*1000 if selected_game == 'minecraft' else 0)
                    except Exception:
                        pass
                    
                    # reset local game state before entering GAME
                    try: reset_match_state()
                    except Exception: pass
                    
                    # host starts game locally too
                    if selected_game == 'piano':
                        ...
                        try:
                            battleship_game = BattleshipGame(net_manager)
                            battleship_game.start_placement(WIDTH, HEIGHT, cell_size=30)
                        except Exception: battleship_game = None
                    elif selected_game == 'penaltis':
                        try:
                            penalties_game = PenaltiesGame(net_manager, is_host=is_host, players=players_list if 'players_list' in locals() else None)
                            battleship_game = penalties_game
                            if battleship_game: battleship_game.start_placement(WIDTH, HEIGHT, cell_size=30)
                        except Exception: battleship_game = None
                        ...
                        for p in players_list:
                            if p != net_manager.peer_id: players[p] = Player()
                        restore_chests()
                        screen = set_opengl_mode()
                ...
                # Handle Minecraft inputs if in minecraft mode
                if selected_game == 'minecraft':
                    if event.type == KEYDOWN:
                        if event.key == K_a: input_dx = -1
                            ...
                            if crafting_menu: crafting_menu.toggle()
                        ...
                        if renderer and world and net_manager.peer_id in players:
                            cam_x, cam_y = renderer.get_camera_pos(players[net_manager.peer_id])
                            mouse_x, mouse_y = pygame.mouse.get_pos()
                            world_x = int((mouse_x + cam_x) // BLOCK_SIZE_PX)
                            world_y = int((HEIGHT - mouse_y + cam_y) // BLOCK_SIZE_PX)
                            if event.button == 1: # Left click: break block
                                if world.get_block(world_x, world_y) != B_AIR:
                                    world.set_block(world_x, world_y, B_AIR)
                                    net_manager.send_event("BLOCK_UPDATE", x=world_x, y=world_y, type=B_AIR)
                            elif event.button == 3: # Right click: place block
                                b_type = players[net_manager.peer_id].selected_item
                                if world.get_block(world_x, world_y) == B_AIR:
                                    world.set_block(world_x, world_y, b_type)
                                    net_manager.send_event("BLOCK_UPDATE", x=world_x, y=world_y, type=b_type)
        ...
        while not msg_queue.empty():
            ...
            if action == "LATE_JOIN_SYNC":
                ...
                if world and x is not None and y is not None:
                    ...
                    if b_type == B_CHEST:
                        chest_id = f"chest_{x}_{y}_{sender}"
                        if chest_id not in chests:
                            chests[chest_id] = Chest(chest_id, sender, position=(x, y))
                ...
                if cid and owner and pos:
                    if cid not in chests:
                        chests[cid] = Chest(cid, owner, position=(pos[0], pos[1]))
                    room_key = room_hash_display
                    if room_key not in saved_chests: saved_chests[room_key] = {}
            ...
            if msg.get("action") == "START_GAME":
                ...
                if g == 'minecraft':
                    ...
                    try:
                        players_in_msg = msg.get('players')
                        penalties_game = PenaltiesGame(net_manager, is_host=False, players=players_in_msg if isinstance(players_in_msg, list) else None)
                        battleship_game = penalties_game
                        if battleship_game: battleship_game.start_placement(WIDTH, HEIGHT, cell_size=30)
                    except Exception: battleship_game = None
                elif g == 'karting':
        ...
        if current_state == STATE_MENU:
            ...
            if selected_game == 'minecraft':
                ...
                if net_manager and net_manager.peer_id in players:
                    p = players[net_manager.peer_id]
                    p.update(world, dt, dx=input_dx, jump=input_jump)
                    
                    # Enviar posición periódicamente
                    now = pygame.time.get_ticks()
                    if now - last_move_send > 50:
                        net_manager.send_event("PLAYER_MOVE", x=p.x, y=p.y, vx=p.vx, vy=p.vy)
                        last_move_send = now

                if renderer:
                    renderer.render(world, players, net_manager.peer_id if net_manager else None)
                    
                    # UI de Minecraft sobre OpenGL (si se requiere PyGame overlay)
                    if input_inventory_open and net_manager and net_manager.peer_id in players:
                        renderer.draw_inventory(screen, players[net_manager.peer_id])
                    if crafting_menu and crafting_menu.active:
                        crafting_menu.draw(screen)
                    if input_tab_held:
                        renderer.draw_player_list(screen, list(players.keys()))

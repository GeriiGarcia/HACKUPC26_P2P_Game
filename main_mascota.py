...
class MascotaGame:
    def __init__(self, screen, net_manager, clock, room_hash=""):
        self.screen = screen
        ...
        # Game State
        self.renderer = None
        self.pets = {} # peer_id -> Pet object
        self.last_move_send = 0
        self.saved_pets = load_all_pets()  # {room_hash: {peer_name: {pet data}}}
    ...
    def save_my_pet(self):
        ...
        if room_key not in self.saved_pets:
            self.saved_pets[room_key] = {}
        ...
        if self.net.peer_id in self.pets:
            ...
            if encrypted:
                self.saved_pets[room_key][self.net.peer_id] = {"encrypted": encrypted}
            else:
                self.saved_pets[room_key][self.net.peer_id] = pet_data
        
        # Guardar mascotas de OTROS jugadores en texto plano
        for pid, p in self.pets.items():
            if pid != self.net.peer_id:
                self.saved_pets[room_key][pid] = p.serialize_state()
    ...
    def on_peer_connected(self, peer_id):
        ...
        self.net.send_event("LATE_JOIN_SYNC", 
                               target_peer=peer_id, 
                               players=list(self.pets.keys()), 
                               pets_data=pet_data)
    ...
    def update(self, dt):
        ...
        if my_pet:
            now = time.time()
            if now - self.last_move_send > 0.1: # Sync 10 veces por sec
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if current_state == STATE_GAME: game_instance.handle_event(event)
                ...
                if btn_create_confirm.handle_event(event):
                    ...
                    net_manager.on_message_received = on_message_received; net_manager.start(); current_state = STATE_ROOM_CREATED
                ...
                if btn_join_confirm.handle_event(event):
                    ...
                    net_manager.on_message_received = on_message_received; net_manager.start(); current_state = STATE_LOBBY
                ...
                if is_host and btn_start_lobby.handle_event(event):
                    net_manager.send_event("START_GAME", players=list(net_manager.peers.keys()))
                    game_instance = MascotaGame(screen, net_manager, clock, room_hash=room_hash_display)
        ...
        if current_state == STATE_GAME:
            game_instance.update(clock.tick(FPS)/1000.0); game_instance.draw()
        else:

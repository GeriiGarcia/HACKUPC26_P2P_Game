...
class HeadSoccerGame:
    def __init__(self, net_manager=None, room_name="HeadSoccerRoom", peer_id=None):
        pygame.init()
        ...
        # P2P Network
        self.own_network = False
        if net_manager is None:
            self.peer_id = peer_id or f"HS_{str(uuid.uuid4())[:6]}"
            self.net = NetworkManager(room_name, self.peer_id)
            self.own_network = True
        else:
            self.net = net_manager
            self.peer_id = getattr(self.net, 'peer_id', peer_id or f"HS_{str(uuid.uuid4())[:6]}")
        ...
        # If this HeadSoccerGame is created with an existing NetworkManager that
        # already has peers connected (e.g., started from the lobby), pre-populate
        # the players and transition to PLAYING immediately instead of waiting
        # for a new on_peer_connected callback.
        try:
            existing_peers = getattr(self.net, 'peers', {}) or {}
            # pick first peer that isn't us
            for pid in existing_peers.keys():
                if pid != self.peer_id:
                    self.other_peer_id = pid
                    break
            if self.other_peer_id:
                self.is_host = self.peer_id < self.other_peer_id
                self.players[self.peer_id] = Player(0, 0, True if self.is_host else False, self.peer_id)
                self.players[self.other_peer_id] = Player(0, 0, False if self.is_host else True, self.other_peer_id)
